...
export class MongoDbProductRepository implements ProductRepository {
    ...
    async startupAsync(): Promise<void> {
        ...
        const colIndexFound = collections.findIndex((c) =>
            c.name.isEqual(MONGO_DB_CONSTANT.COLLECTION_PRODUCTS),
        )
        ...
        if (colIndexFound < 0) {
            ...
            console.log(
                `${MONGO_DB_CONSTANT.COLLECTION_PRODUCTS} db collection indexes created: ${indexCreatedResult} `,
            )
            ...
            console.log(
                `${MONGO_DB_CONSTANT.COLLECTION_PRODUCTS} db collection indexes created: ${indexCreatedResult1} `,
            )
            ...
            console.log(
                `${MONGO_DB_CONSTANT.COLLECTION_PRODUCTS} db collection indexes created: ${indexCreatedResult2} `,
            )
        }
    ...
    async loadByBusinessUnitIdAsync(businessUnitId: ObjectId): Promise<Product[]> {
        ...
        const cursor = this.productCollection.find(query).sort(sort)
    ...
    describe("#product-repositories.ts", () => {
        ...
        test(
            ...
            async () => {
                ...
                const mockProduct = async (): Promise<Product> => {
                    return {
                        ...
                        name: `${testHelper.generateRandomString(
                            5,
                        )} ${testHelper.generateRandomString(5)}`,
                ...
                const objId = await productRepository.saveAsync(
                    await mockProduct(),
                    AppSettings.instance.systemId,
                )
                const objId2 = await productRepository.saveAsync(
                    await mockProduct(),
                    AppSettings.instance.systemId,
                )

                const product = await productRepository.loadOneAsync(new ObjectId(objId))
                expect(product.id).toEqual(objId.toHexString())

                const products = await productRepository.loadByBusinessUnitIdAsync(
