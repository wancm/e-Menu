...
export class MongoDbPersonRepository implements PersonRepository {
    ...
    async startupAsync(): Promise<void> {
        ...
        const colIndexFound = collections.findIndex((c) =>
            c.name.isEqual(MONGO_DB_CONSTANT.COLLECTION_PERSONS),
        )
        ...
        if (colIndexFound < 0) {
            ...
            console.log(
                `${MONGO_DB_CONSTANT.COLLECTION_PERSONS} db collection indexes created: ${indexCreatedResult} `,
            )
            ...
            console.log(
                `${MONGO_DB_CONSTANT.COLLECTION_PERSONS} db collection indexes created: ${indexCreatedResult1} `,
            )
            ...
            console.log(
                `${MONGO_DB_CONSTANT.COLLECTION_PERSONS} db collection indexes created: ${indexCreatedResult2} `,
            )
    ...
    async loadByBusinessUnitIdAsync(businessUnitId: string): Promise<Person[]> {
        ...
        const cursor = this.personCollection.find(query).sort(sort)
    ...
    async existsEmailAsync(email: string): Promise<boolean> {
        ...
        return (await this.personCollection.countDocuments(query, { limit: 1 })) > 0
    }

    async findByQueryTextAsync(
    ...
    describe("#person-repositories.ts", () => {
        ...
        test(
            ...
            async () => {
                ...
                const mockPerson = async (): Promise<Person> => {
                    return {
                        businessUnitId,
                        email: `${testHelper.generateRandomString(
                            5,
                        )}@${testHelper.generateRandomString(3)}.com`,
                ...
                const objId = await personRepository.saveAsync(
                    await mockPerson(),
                    AppSettings.instance.systemId,
                )
                const objId2 = await personRepository.saveAsync(
                    await mockPerson(),
                    AppSettings.instance.systemId,
                )

                const persons = await personRepository.loadByBusinessUnitIdAsync(businessUnitId)
        ...
        test(
            ...
            async () => {
                ...
                const email = `${testHelper.generateRandomString(
                    5,
                )}@${testHelper.generateRandomString(3)}.com`
                ...
                const exists = await personRepository.existsEmailAsync(email)

                const person = await personRepository.findByEmailAsync(email)
        ...
        test(
            ...
            async () => {
                ...
                for (let i = 0; i < counter; i++) {
                    ...
                    await personRepository.saveAsync(person, AppSettings.instance.systemId)
         ...
         */
                    switch (random) {
                        ...
                        case 4:
                            ...
                            queryText = person.contact?.addresses?.map((a: any) => a)[0].line1
                            break

                        case 5:
                            queryType = "addresses.line2"
                            queryText = person.contact?.addresses?.map((a: any) => a)[0].line2
                            break

                        case 6:
                            queryType = "addresses.line3"
                            queryText = person.contact?.addresses?.map((a: any) => a)[0].line3
                            break

                        case 7:
                            queryType = "addresses.state"
                            queryText = person.contact?.addresses?.map((a: any) => a)[0].state
                            break

                        case 8:
                            queryType = "addresses.city"
                            queryText = person.contact?.addresses?.map((a: any) => a)[0].city
                            break

                        case 9:
                            queryType = "addresses.countryCode"
                            queryText = person.contact?.addresses?.map((a: any) => a)[0].countryCode
                            break

                        case 0:
                            queryType = "addresses.countryCode"
                            queryText = person.contact?.phones
                                ?.map((a: any) => a)[0]
                                .number?.toString()
                            break
                    }
                    ...
                    console.time(label)
                    const persons = await personRepository.findByQueryTextAsync(
                        businessUnitId,
                        queryText ?? "",
                    )
